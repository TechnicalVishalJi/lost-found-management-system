    ...
    <div className={`item-card ${item.Type.toLowerCase()} fade-in`}>
      ...
      {isOwner && (
        <div className="item-actions">
          <button className="btn" style={{padding: '0.5rem', background: 'var(--bg-color)'}} onClick={() => onEdit(item)}>
            <Edit size={16} />
          </button>
          <button className="btn" style={{padding: '0.5rem', background: '#FEE2E2', color: 'var(--error-color)'}} onClick={() => onDelete(item._id)}>
            <Trash2 size={16} />
          </button>
        </div>
      )}
